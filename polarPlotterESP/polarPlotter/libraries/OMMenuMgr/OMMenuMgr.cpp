...
void OMMenuMgr::_menuNav(uint8_t p_mode) {
	...
	uint8_t childCount = m_curParent->targetCount;
    childCount--;
...
void OMMenuMgr::_activate(OMMenuItem* p_item, bool p_return) {
    ...
    if( type == ITEM_VALUE ) {
    	...
    	f_valueHandler callback = reinterpret_cast<f_valueHandler>( p_item->target) ;
...
void OMMenuMgr::_displayList(OMMenuItem* p_item, uint8_t p_target) {
    ...
    OMMenuItem** items = reinterpret_cast<OMMenuItem**>((void*)(p_item->target));
    //Serial.println("_displayList s1");
    //void* c = (void*)p_item->target;
    // Serial.println(*c);
    //OMMenuItem** items = reinterpret_cast<OMMenuItem**>(c);

    //m_curSel = reinterpret_cast<OMMenuItem*>(pgm_read_word(&(items[p_target])));
    m_curSel = reinterpret_cast<OMMenuItem*>(items[p_target]);

    //Serial.println("%p", (void *)**items);
    //Serial.println(*items);
    //Serial.println((long unsigned int)(void**)items);
    //Serial.println((long unsigned int)(void*)*items);
    //Serial.println((long unsigned int)(void)**items);
    //Serial.println(p_target);
    //OMMenuItem* a = items[p_target];
    //Serial.println("_displayList s1c");
    //short unsigned int b =pgm_read_word(&a);
    //Serial.println("_displayList s1d");
    //m_curSel = reinterpret_cast<OMMenuItem*>(b);
    //Serial.println("_displayList s2");
...
void OMMenuMgr::_displayEdit(OMMenuItem* p_item) {
    ...
    OMMenuValue* value = reinterpret_cast<OMMenuValue*>((void*)(p_item->target));
    ...
    void* valPtr  = reinterpret_cast<void*>((void*)value->targetValue);
    ...
    if( type == TYPE_BYTE )
        ...
        OMMenuSelectListItem** list = reinterpret_cast<OMMenuSelectListItem**>((void*)(sel->list));
        //uint8_t              curVal = *reinterpret_cast<uint8_t*>(pgm_read_word(&(sel->targetValue)));
        uint8_t              curVal = *reinterpret_cast<uint8_t*>(sel->targetValue);
        //uint8_t              count  = pgm_read_byte(&(sel->listCount));
        uint8_t              count  = sel->listCount;
        ...
        for(uint8_t i = 0; i < count; i++ ) {
        	...
        	OMMenuSelectListItem* item = reinterpret_cast<OMMenuSelectListItem*>(list[i]);
            //uint8_t tgt = pgm_read_byte(&(item->value));
        	uint8_t tgt = item->value;
            if( tgt == curVal )
                m_temp = i;
        }
        ...
        uint8_t*       target = reinterpret_cast<uint8_t*>(flag->flag);
...
void OMMenuMgr::_displaySelVal(OMMenuSelectListItem** p_list, uint8_t p_idx) {
    ...
    OMMenuSelectListItem* item = reinterpret_cast<OMMenuSelectListItem*>(p_list[p_idx]);
...
void OMMenuMgr::_modifySel(OMMenuValue* p_value, uint8_t p_mode) {
	...
	OMMenuSelectValue*      sel = reinterpret_cast<OMMenuSelectValue*>((word)p_value->targetValue);
    //uint8_t               count = pgm_read_byte(&(sel->listCount));
	uint8_t               count = sel->listCount;
    //OMMenuSelectListItem** list = reinterpret_cast<OMMenuSelectListItem**>(reinterpret_cast<void*>(pgm_read_word(&(sel->list))));
	OMMenuSelectListItem** list = reinterpret_cast<OMMenuSelectListItem**>((void*)sel->list);
...
//void OMMenuMgr::_edit(OMMenuItem* p_item, uint8_t p_type) {
	...
	OMMenuValue* thisValue = reinterpret_cast<OMMenuValue*>((word)p_item->target);

    //uint8_t type = pgm_read_byte(&(thisValue->type));
	uint8_t type = thisValue->type;
    //long    min  = pgm_read_dword(&(thisValue->min));
	long    min  = thisValue->min;
    //long    max  = pgm_read_dword(&(thisValue->max));
	long    max  = thisValue->max;
    ...
    //else if( p_type == CHANGE_UP || p_type == CHANGE_DOWN || p_type == MODE_NOOP ) {
    	...
    	void* ptr = reinterpret_cast<void*>((word)thisValue->targetValue);
        ...
        if( type == TYPE_SELECT ) {
            ...
            OMMenuSelectListItem* item  = reinterpret_cast<OMMenuSelectListItem*>(list[m_temp]);
            //uint8_t newVal              = pgm_read_byte(&(item->value));
            uint8_t newVal              = item->value;
            //uint8_t* real               = reinterpret_cast<uint8_t*>(pgm_read_word(&(sel->targetValue)));
            uint8_t* real               = reinterpret_cast<uint8_t*>(sel->targetValue);
            *real                       = newVal;
