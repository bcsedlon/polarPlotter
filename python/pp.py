...
class pp:
    ...
    R_STEP = 1.8 * math.pi / 180.0 / 100 # step angle [radians]
    D_STEP = 1.0 / 100 # step distance [mm]
    ...
    def gotoXYlog(self, x, y):
        self.gotoXY(x, y)
        x, y = self.getXYself()
        writeHexLR(self.dSteps, self.rSteps, self.s)
        writeHexXY(x, y, self.s)
    
    def gotoXY(self, x, y):
        d = math.sqrt(x * x + y * y)
        self.gotoDsteps(d /  self.D_STEP)
        if y == 0:
            #r = 0.0
            r = math.pi / 2
            if x < 0:
    ...
    def getXYself(self): 
        return self.getXY(self.r, self.d)
        
    def gotoR(self, r):
        self.r = r
        self.rSteps = int(self.r / self.R_STEP)

    def gotoD(self, d):
        self.d = d
        self.dSteps = int(self.d / self.D_STEP)
        
    def gotoRsteps(self, rSteps):
        self.rSteps = int(rSteps)
        self.r = self.rSteps * self.R_STEP
            
    def gotoDsteps(self, dSteps):
        self.dSteps = int(dSteps)
        self.d = self.dSteps * self.D_STEP
        
    def plottSVGlines(self, paths, x0, y0, callback):
        xMax = 0
        xMin = 0
        yMax = 0
        yMin = 0
        ...
        width = xMax - xMin
        height = yMax - yMin
        svgScale = (X_BASE * PYGAME_RATIO) / width
        x0svg = int(width / 2)
        y0svg = int(height / 2)
            
        for path in paths:
            self.movePen(SPRAY_STOP)
            l =  path.length(error=1e-5)
            for i in range(0, int(l)):
                if heardEnter():
                    return
                point = path.point(i / l)
                x = (point.real - x0svg) * svgScale
                y = (point.imag - y0svg) * svgScale 
                self.gotoXYlog(x, y)
                x, y = self.getXYself()
                self.movePen(SPRAY_START)
                if i > 0:
                    callback((x0 + xLast), (y0 + yLast), (x0 + x) , (y0 + y))
                xLast = x
                yLast = y
        self.movePen(SPRAY_STOP)
    
    def plottHexXY(self, filename, scale, x0, y0, callback):
        xLast = 0
        yLast = 0
    ...
    def plottXY(self, filename, scale, x0, y0, callback):
        xLast = 0
        yLast = 0
        with open(filename, 'rb') as file:
            #mode = struct.unpack('i', file.read(4))
    ...
    def plottHexLR(self, filename, scale, x0, y0, callback):
        xLast = 0
        yLast = 0
        with open(filename, 'rb') as file:
            #mode = struct.unpack('i', file.read(4))
...
class gui:
    ...
    def displayMenu():
        ...
        #                break
            ...
            print "51 - pp PNG" 
            print "52 - pp SVG" 
            print "53 - pp plottHEX XY" 
            print "54 - pp plottHEX LR" 
            print "55 - pp plott XY"
            ...
            def drawCallback(x0, y0, x1, y1):
                outputMap.sprayLine(x0, y0, x1, y1)
                pygameGui.sprayLine(x0, y0, x1, y1)
            ppX0 = (int(X_BASE * PYGAME_RATIO)) 
            ppY0 = (int(Y_BASE * PYGAME_RATIO)) 
            pp0 = pp()
            ...
            if displayMenu == 50:
                ...
                for i in range(1, 1000000, 1):
                    if heardEnter():
                        return
                    pp0.gotoRsteps(i)
                    pp0.gotoDsteps(int(i / 100))
                    x, y = pp0.getXYself()
                    pygameGui.spray(ppX0 + x, ppY0 + y)
                    #print('X: {}, Y: {}'.format(x, y))
                    #time.sleep(0.1)
                    #outputMap.draw.ellipse((X_SIZE / 2 + x - SPRAY_D, Y_SIZE / 2 + y - SPRAY_D, X_SIZE / 2 + x + SPRAY_D, Y_SIZE / 2 + y + SPRAY_D), fill = 'blue', outline ='blue')
            ...
            if displayMenu == 51:
                ...
                rgbImage = inputPattern.getImage(filename) 
                width, height = rgbImage.size
                rgbScale = width / (X_BASE * PYGAME_RATIO)
                ...
                for i in range(1, 1000000, 1):
                    if heardEnter():
                        break
                    pp0.gotoRsteps(i)
                    pp0.gotoDsteps(int(i / 200))
                    x, y = pp0.getXYself()
                    x1 = width / 2 + x * rgbScale
                    y1 = height / 2 + y * rgbScale
                    #print('x: {} y: {}'.format(x1, y1))
                    if 0 <= x1 < width:
                        if 0 <= y1 < height:
                            r, g, b = rgbImage.getpixel((x1, y1))
                            l = (r, g, b)
                            c = sum(l) / len(l)
                            if c < PP_COLOR_LIMIT:
                                pygameGui.spray(ppX0 + x, ppY0 + y)
            ...
            if displayMenu == 52:
                '''
                try:
                    k = int(raw_input("> Set k: "))
                except:
                    k = 1
                print(math.sqrt(k * k + k * k))
                pp0.gotoXY(k, k)
                pp0.gotoXY(k, -k)
                pp0.gotoXY(-k, -k)
                pp0.gotoXY(-k, k)
                return
                '''
                fileName = inputPatternSVG.getFilename('./svg') 
                if fileName == 0:
                    return
                ...
                logFileXY = open(fileName + '.xy', 'w')
                logFileLR = open(fileName + '.lr', 'w')
    
                pp0.plottSVGlines(inputPatternSVG.getSVGpaths(fileName), ppX0, ppY0, drawCallback)                
            ...
            if displayMenu == 53:
                ...
                pygameGui.reset()
                pygameGui.drawAxix()
                pp0.plottHexXY(filename, s, ppX0, ppY0, drawCallback)             
            ...
            if displayMenu == 54:
                ...
                pygameGui.reset()
                pygameGui.drawAxix()
                pp0.plottHexLR(filename, s, ppX0, ppY0, drawCallback)   
            ...
            if displayMenu == 55:
                ...
                pygameGui.reset()
                pygameGui.drawAxix()
                pp0.plottXY(filename, s, ppX0, ppY0, drawCallback)                      
...
class outputMap:
    ...
    @staticmethod
    def initBlank():
        outputMap.img = Image.new("RGB", (X_BASE, Y_BASE), "#D0D0D0")
        outputMap.draw = ImageDraw.Draw(outputMap.img)
    ...
    @staticmethod
    def drawAxix():
        outputMap.sprayLine(0, int(Y_BASE / 2), int(X_BASE), int(Y_BASE / 2))
        outputMap.sprayLine(int(X_BASE / 2), 0, int(X_BASE / 2), int(Y_BASE))
...
class pygameGui:
    ...
    @staticmethod
    def drawAxix():
        pygameGui.sprayLine(0, int(Y_BASE / 2), int(X_BASE), int(Y_BASE / 2))
        pygameGui.sprayLine(int(X_BASE / 2), 0, int(X_BASE / 2), int(Y_BASE))
